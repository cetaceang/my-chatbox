...
function initializeEventListeners() {
    const newConversationBtns = document.querySelectorAll('.new-conversation-btn');
    if (newConversationBtns) {
        ...
        fileUploadInput.onchange = function() {
            if (fileUploadInput.files.length > 0) {
                ...
                sendButton.onclick = function() {
                    // 关键修复：生成单个UUID，同时用作tempId和generationId
                    const uniqueId = window.generateUUID();
                    
                    // 调用通用处理函数
                    handleImageUpload(file, { tempId: uniqueId, generationId: uniqueId });
                    
                    // 清理UI
                    previewContainer.remove();
                    fileUploadInput.value = '';
                    // 注意：不要清空messageInput.value，让handleImageUpload函数处理
            ...
            // Save Edit Button
                ...
                try {
                    ...
                    if (window.ChatState.getMessage(editInfo.id)) {
                        ...
                        // Fallback for messages not in ChatState - this is an unexpected state.
                        // Instead of calling a non-existent function, log an error and revert the UI.
                        console.error(`[Save Edit] Message ID ${editInfo.id} not found in ChatState. This is an inconsistent state. Reverting UI and aborting.`);
                        alert("出现状态不一致错误，无法保存编辑。建议刷新页面。");

                        // Safely revert the UI to its pre-edit state
                        try {
                            const messageState = window.ChatState.getMessage(editInfo.id);
                            if (messageState) {
                                window.MessageFactory.updateDOMForMessage(editInfo.id);
                            } else {
                                // Fallback to rebuild if still not in state (which is expected here)
                                rebuildMessageElement(messageDiv, editInfo.id, editInfo.content, editInfo.isUser);
                            }
                        } catch (revertError) {
                            console.error('Error reverting UI after inconsistent state:', revertError);
                            // If reverting fails, a reload is the safest option.
                            window.location.reload();
                        }
                    }
